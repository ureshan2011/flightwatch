...
from . import DOCS_DIR, ROOT_DIR, storage, predict, analytics
...
def _load_firebase_config():
    """The PUBLIC Firebase web config, resolved from (in priority order):

      1. the ``FARO_FIREBASE_CONFIG`` env var (a GitHub repo Variable holding the
         firebaseConfig JSON), then
      2. the committed ``firebase.web.json`` at the repo root.

    Returns ``(cfg, vapid)`` -- the firebaseConfig dict (with any ``vapidKey``
    split out) and the FCM web-push VAPID key (env ``FARO_FCM_VAPID_KEY`` wins,
    else ``vapidKey`` from the config). ``cfg`` stays ``{}`` until a real
    ``apiKey`` is present, so the $0, no-account demo build is unchanged until the
    project is actually filled in. The web config is NOT secret -- it only
    identifies the project; security is enforced by Firestore rules + Auth.
    """
    cfg = {}
    ...
    if not cfg.get("apiKey"):
        path = os.path.join(ROOT_DIR, "firebase.web.json")
    ...
    vapid = ((os.environ.get("FARO_FCM_VAPID_KEY") or "").strip()
             or str(cfg.get("vapidKey") or "").strip())
    # Drop non-firebaseConfig keys (vapidKey, and any _comment/_note doc fields) so
    # only the real config reaches initializeApp() and the client global.
    cfg = {k: v for k, v in cfg.items() if k != "vapidKey" and not k.startswith("_")}
    return cfg, vapid
...
def _write_firebase_sw():
    ...
    if not cfg.get("apiKey"):
        # Keep the published site clean if the config is absent/removed.
        try:
            os.remove(sw_path)
        except OSError:
            pass
